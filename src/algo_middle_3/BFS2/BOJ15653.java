...
class PairC {
    ...
    PairC(boolean first, boolean second) {
        this.first = first;
        this.second = second;
    }
}
...
public class BOJ15653 {
    ...
    static PairC simulate(char[][] a, int k, Point p) {
        int x = p.x;
        int y = p.y;
        if (a[x][y] == '.') return new PairC(false, false);
        int n = a.length;
        int m = a[0].length;
        boolean moved = false;
        while (true) {
            ...
            if (ch == '#') {
                ...
                p.x = x;
                p.y = y;
                return new PairC(moved, true);
            }
        }
    }

    static PairC next(char[][] a, Points p, int dir) {
        int n = a.length;
        int m = a[0].length;
        char[][] b = new char[n][m];
        for (int i = 0; i < n; i++) {
        ...
        while (true) {
            PairC p1 = simulate(b, dir, red);
            PairC p2 = simulate(b, dir, blue);
