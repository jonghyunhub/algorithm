...
public class BOJ16236 {
    ...
    static PairB bfs(int[][] a, int sx, int sy, int size) {
        int n = a.length;
        ArrayList<PairB> ans = new ArrayList<>();
        int[][] d = new int[n][n];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                d[i][j] = -1;
            ...
            for (int k = 0; k < 4; k++) {
                ...
                if (0 <= nx && nx < n && 0 <= ny && ny < n && d[nx][ny] == -1) {
                    ...
                    if (ok) {
                        ...
                        d[nx][ny] = d[x][y] + 1;
                        if (eat) {
                            ans.add(new PairB(d[nx][ny], nx, ny));
                        }
                    }
                }
            }
