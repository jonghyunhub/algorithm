...
class PairA implements Comparable<PairA> {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof PairA) {
            PairA that = (PairA) obj;
    ...
    public int compareTo(PairA that) {
...
public class BOJ16952 {
    ...
    public static void main(String[] args) {
        ...
        int n = sc.nextInt();
        PairA[][][][] d = new PairA[n][n][n * n][3];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < n * n; k++) {
                    for (int l = 0; l < 3; l++) {
                        d[i][j][k][l] = new PairA(-1, -1);
                    }
                }
            }
        }
        ...
        PairA ans = new PairA(-1, -1);
        while (!q.isEmpty()) {
            int x = q.remove();
            int y = q.remove();
            int num = q.remove();
            int piece = q.remove();
            PairA p = d[x][y][num][piece];
            if (num == n * n - 1) {
            ...
            for (int i = 0; i < 3; i++) {
                if (piece == i) continue;
                PairA np = new PairA(p.dist + 1, p.change + 1);
            ...
            if (piece == 0) { // knight
                for (int k = 0; k < 8; k++) {
                    ...
                    if (0 <= nx && nx < n && 0 <= ny && ny < n) {
                        ...
                        PairA np = new PairA(p.dist + 1, p.change);
